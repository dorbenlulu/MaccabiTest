...
const Clients = observer(() => {
  ...
  const handleSelectAllClick = event => {
    if (event.target.checked) {
      const newSelecteds = clients.list.map(n => n.name);
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={clients.list.length}
            />
            <TableBody>
              {stableSort(clients.list, getComparator(order, orderBy))
                ...
                .map((client, index) => {
                  ...
                  return (
                    <TableRow
                      hover
                      onClick={event => handleClick(event, client.fullName)}
                      role="checkbox"
                      aria-checked={isItemSelected}
                      tabIndex={-1}
                      key={client.fullName}
                    >
                      <TableCell align="center" component="th" id={labelId} scope="client" padding="none">
                        {client.fullName}
                      </TableCell>
